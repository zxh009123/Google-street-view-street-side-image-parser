...
class CustomeAPI:
  ...
  def download_links(self, dir_path, prefix, metadata_file='metadata.json', metadata_status='status', status_ok='OK'):
    ...
    for i, url in enumerate(self.links):
      if metadata[i][metadata_status] == status_ok:
        file_path = path.join(dir_path,  prefix + '_' + str(i) + '.jpg')
  ...
  def get_lat_lon(self):
    ...
    for i in self.metadata:
      if i['status'] == 'OK':
        coord = [i["location"]["lat"], i["location"]["lng"]]
        list_lat_lon.append(coord)
    return list_lat_lon

  def set_heading(self, headings):
    assert len(headings) <= len(self.params)
    for i in range(len(headings)):
      self.params[i]['heading'] = str(headings[i])
